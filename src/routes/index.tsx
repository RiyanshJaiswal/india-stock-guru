...
import { defaultWatchlist, stocks, type Stock } from "@/data/market";
  ...
  head: () => ({
    meta: [
      { title: "Dalal Desk — AI Indian Stock Market Dashboard" },
      {
        name: "description",
        content:
          "Track NSE and BSE stocks, your watchlist, portfolio P&L, market news and an AI market copilot in one dark-themed dashboard.",
      },
      { property: "og:title", content: "Dalal Desk — AI Indian Stock Market Dashboard" },
      {
        property: "og:description",
        content:
          "Live-style NSE/BSE watchlist, portfolio P&L, market news and an AI copilot in one dark dashboard.",
      },
...
function Dashboard() {
  ...
  const activeStock = useMemo<Stock>(
    () => stocks.find((s) => s.symbol === activeSymbol) ?? stocks[0]!,
    [activeSymbol],
  );
    ...
    <div className="min-h-screen">
      ...
      <main className="mx-auto max-w-7xl space-y-4 px-4 py-5 sm:px-6 sm:py-6">
        <StockSearch
          ...
          onSelect={(stock) => setActiveSymbol(stock.symbol)}
        />
        ...
        <div className="grid gap-4 lg:grid-cols-3">
          ...
          <div className="space-y-4">
            ...
            />
            <AiAssistant activeSymbol={activeSymbol} />
          </div>
        </div>
        ...
        <p className="pb-6 text-center text-xs text-muted-foreground">
          Demo data for personal use. Wire the FastAPI backend and Lovable Cloud for live quotes,
