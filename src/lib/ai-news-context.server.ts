...
function evidenceMeta(item: MarketNewsItem): { evidenceStatus: "FACT" | "INFERENCE" | "UNKNOWN"; evidenceBasis: string } {
  ...
  if (hasHeadline && hasSource && hasDate) {
    return {
      evidenceStatus: "FACT",
      evidenceBasis: "Headline is directly attributed to a named source with a published timestamp.",
    };
  }
  return {
    evidenceStatus: "UNKNOWN",
    evidenceBasis: "Source/date evidence is incomplete; do not treat this item as a verified current fact.",
  };
}
...
function toContext(items: MarketNewsItem[]): AiNewsContext[] {
  return items
    ...
    .map(({ headline, source, publishedAt, tickers, sentiment, primaryEventType, impactDirection, impactLevel, impactScore, timeHorizon, confidence, impactReason }) => {
      const meta = evidenceMeta({ headline, source, publishedAt, tickers, sentiment, primaryEventType, impactDirection, impactLevel, impactScore, timeHorizon, confidence, impactReason } as MarketNewsItem);
      return {
        headline,
        source,
        publishedAt,
