...
function safeProviderMessage(error: unknown, fallback: string): string {
  const raw = error instanceof Error ? error.message : String(error ?? "");
  const cleaned = raw.replace(/\s+/g, " ").trim();
  if (!cleaned) return fallback;
  // Never let a provider response/serialized payload blow out the UI.
...
export const getHistory = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<HistoryResult> => {
    ...
    try {
      ...
      if (candles.length === 0) {
        return { ok: false, error: { code: "NO_HISTORY", symbol: data.symbol, message: `No historical candles available for ${data.symbol}.` } };
      }
      ...
      return {
        ...
        error: {
          code: "PROVIDER_ERROR",
          symbol: data.symbol,
          message: safeProviderMessage(error, "Historical data provider failed."),
        },
      };
    }
  });
...
export const getTechnicalAnalysis = createServerFn({ method: "GET" })
  ...
  .handler(async ({ data }): Promise<TechnicalAnalysisResult> => {
    ...
    let candles: Candle[];
    try {
      candles = await providerHistory(data.symbol, data.interval as Interval, data.range as Range);
    } catch (error) {
      return {
        ok: false,
        error: {
