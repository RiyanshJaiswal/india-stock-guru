...
export function Portfolio() {
  ...
  const quoteOptions = quotesQuery(symbols);
  ...
  const rows = positions.map((position) => {
    const quote = data?.find((q) => q.symbol.trim().toUpperCase() === position.symbol.trim().toUpperCase());
    ...
    const suggestion = STOCK_SUGGESTIONS.find((stock) => stock.symbol.toUpperCase() === position.symbol.toUpperCase());
    return {
      ...position,
      ltp,
  ...
  const suggestions = useMemo(() => {
    ...
    if (!query || suggestionSelected) return [];

    const local = STOCK_SUGGESTIONS.filter(
      (stock) =>
        stock.name.toLowerCase().includes(query) ||
        stock.symbol.toLowerCase().replace(/\.ns$|\.bo$/i, "").includes(query),
    );
    const remote = (remoteSuggestions ?? []).map((stock) => ({ symbol: stock.symbol, name: stock.name }));
    const merged = [...remote, ...local];
    const unique = merged.filter((stock, index, list) =>
      list.findIndex((item) => item.symbol.toUpperCase() === stock.symbol.toUpperCase()) === index,
    );
    ...
    return unique
      ...
      .slice(0, 6);
  }, [symbol, suggestionSelected, remoteSuggestions]);
