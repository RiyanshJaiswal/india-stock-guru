...
function formatChartTooltip(value: unknown, name: unknown) {
  ...
  if (["open", "high", "low", "close"].includes(key)) return [formatPrice(value), key[0].toUpperCase() + key.slice(1)];
  if (key.startsWith("sma") || key.startsWith("ema")) return [formatPrice(value), String(name).toUpperCase()];
  return [compactVolume(Number(value)), String(name).toUpperCase()];
}

function OhlcTooltip({ active, payload }: { active?: boolean; payload?: Array<{ payload?: ChartPoint }> }) {
  if (!active || !payload?.length) return null;
  const point = payload[0]?.payload;
  if (!point) return null;
  return (
    <div className="rounded-lg border border-border bg-background px-3 py-2 text-xs shadow-xl">
      <p className="mb-1 font-semibold">{formatTooltipDate(point.time)}</p>
      <div className="grid grid-cols-2 gap-x-4 gap-y-0.5">
        <span className="text-muted-foreground">Open</span><span className="num text-right">{formatPrice(point.open)}</span>
        <span className="text-muted-foreground">High</span><span className="num text-right">{formatPrice(point.high)}</span>
        <span className="text-muted-foreground">Low</span><span className="num text-right">{formatPrice(point.low)}</span>
        <span className="text-muted-foreground">Close</span><span className="num text-right">{formatPrice(point.close)}</span>
        <span className="text-muted-foreground">Volume</span><span className="num text-right">{compactVolume(point.volume)}</span>
      </div>
    </div>
  );
}

export function ChartPanel({ quote, symbol, isLoading, linkToDetails = true, onLatestOpen }: { quote: Quote | null | undefined; symbol: string; isLoading?: boolean; linkToDetails?: boolean; onLatestOpen?: (open: number | null) => void }) {
  ...
  useEffect(() => {
    ...
    getHistory({ data: { symbol, interval: "1d", range: selectedRange } }).then((result) => {
      if (cancelled) return;
      if (!result.ok) { setPoints([]); setHistoryError(result.error.message); onLatestOpen?.(null); return; }
      ...
      setPoints(mapped);
      onLatestOpen?.(mapped.at(-1)?.open ?? null);
    }).catch((error) => { if (!cancelled) { setPoints([]); setHistoryError(error instanceof Error ? error.message : "Unable to load chart data."); onLatestOpen?.(null); } }).finally(() => { if (!cancelled) setHistoryLoading(false); });
    return () => { cancelled = true; };
  }, [symbol, selectedRange, onLatestOpen]);
